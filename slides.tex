...
\begin{frame}{the meaning of a proposition}
  ...
  For a type $A$, implicit in the explanation of $\Force{w}{\IsVer{P}}$ is a
  PER $\VAL{A}{w}\subseteq \Domain{w}\times\Domain{w}$ whose members
  \alert{reflect} the computational content of \emph{verification acts}.
\end{frame}
...
\begin{frame}{the justifications sheaf}
  ...
  Based on this meaning explanation, we can justify the construct a family of
  sheaves $\mathcal{K}_A : \FINSET^{\Opposite{\Worlds}}$, such that
  $\mathcal{K}_A(w)$ is the species of constructions of $\IsTrue{A}$ which have
  been effected at or prior to $w$.
  \pause\bigskip
...
\begin{frame}{eliminating non-determinism with a spread}
  ...
  Idea: reformulate the judgments relative to a choice sequence of ``choosers''.
\end{frame}
